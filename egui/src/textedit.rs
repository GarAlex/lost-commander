...
//!
//! Everything about what the bytes mean is in [`lost_commander_core::textedit`] and
//! [`lost_commander_core::encoding`], tested without a window. What is here is the box you
...
use lost_commander_core::encoding::{self, Encoding};
    ...
    ui.horizontal(|ui| {
        ...
        ui.label(
            RichText::new(format!(
                ...
                lost_commander_core::entry::size_in_words(session.document.size() as u64)
            ))
            .size(11.0)
            .color(theme::text_faint()),
        );
