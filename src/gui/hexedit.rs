...
//! run written back in place - lives in [`crate::hex`] and is tested without a
...
use crate::hex::{self, Cursor, Dump, Edits, Pane};
...
pub fn draw(ctx: &egui::Context, session: &mut Session) -> Outcome {
    ...
    let escaped = super::modal(ctx, "Edit bytes", |ui| {
        ...
        ui.horizontal(|ui| {
            ...
            ui.label(
                RichText::new(crate::entry::size_in_words(session.dump.size))
                    .size(11.0)
